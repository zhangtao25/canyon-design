...
[istanbuljs](https://github.com/istanbuljs/istanbuljs) 是一款优秀的JavaScript代码覆盖率工具，主要用于单元测试的代码覆盖率检测和生成本地覆盖率报告。然而，随着现代前端技术和UI自动化测试的发展，对端到端测试的代码覆盖率检测需求逐渐增加，包括高并发覆盖率上报、实时覆盖率聚合和覆盖率数据的水合展示等功能。  集团内的工具和行业内的开源工具如Istanbuljs，主要支持单体应用的覆盖率收集，对于端到端测试的覆盖率收集支持不足。因此，我们在Istanbuljs的基础上开发了Canyon，一款全面提升JavaScript代码质量的覆盖率分析工具。Canyon不仅能处理高并发的覆盖率上报和实时聚合，还能提供覆盖率报告的水合展示。  目前，携程的多个部门已经开始使用Canyon，并在持续集成流水线构建阶段插入探针代码，以便在UI自动化测试阶段收集和上报覆盖率数据。Canyon服务端能生成详尽的覆盖率报告，为UI自动化测试用例提供全面的覆盖率数据指标。
