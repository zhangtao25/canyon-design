...
携程的前端技术日益精进，有自己的前端流量录制平台，并且部署了相当大规模的模拟器集群进行UI自动化(flybirds)回放，

但是对于端到端测试的代码覆盖率收集支持不足。

传统的istanbuljs提供的功能已经无法满足我们的需求。我们需要处理UI自动化过程中来自前端高并发的覆盖率上报，
实时的覆盖率聚合，以及覆盖率数据的水合展示。因此，我们在Istanbuljs的基础上开发了Canyon，
一款全面提升JavaScript代码质量的覆盖率分析工具。

目前，携程的多个部门已经开始使用Canyon，并在持续集成流水线构建阶段插入探针代码，
以便在UI自动化测试阶段收集和上报覆盖率数据。Canyon服务端能生成详尽的覆盖率报告，为UI自动化测试用例提供全面的覆盖率数据指标。
